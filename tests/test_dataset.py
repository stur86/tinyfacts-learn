...
def test_stride_reduces_window_count(fake_hub):
    ds1 = TinyfactsDataset(context_size=CONTEXT_SIZE, stride=1)
    ds4 = TinyfactsDataset(context_size=CONTEXT_SIZE, stride=4)
    n_tokens = ds1.n_tokens
    assert len(ds1) == n_tokens - CONTEXT_SIZE
    assert len(ds4) == -(-(n_tokens - CONTEXT_SIZE) // 4)  # ceil division


def test_stride_advances_window_start(fake_hub):
    stride = 3
    ds = TinyfactsDataset(context_size=CONTEXT_SIZE, stride=stride)
    x0, _ = ds[0]
    x1, _ = ds[1]
...
def test_non_overlapping_stride_gives_one_pass(fake_hub):
    """stride == context_size means an epoch is one pass over the corpus."""
    ds = TinyfactsDataset(context_size=CONTEXT_SIZE, stride=CONTEXT_SIZE)
    assert len(ds) == -(-(ds.n_tokens - CONTEXT_SIZE) // CONTEXT_SIZE)


def test_rejects_invalid_stride(fake_hub):
    with pytest.raises(ValueError, match="stride must be"):
        TinyfactsDataset(context_size=CONTEXT_SIZE, stride=0)


# ── train/val split ───────────────────────────────────────────────────────────

# Half the rows on each side, so both are non-empty with only four fake rows
SPLIT_KWARGS = dict(context_size=CONTEXT_SIZE, val_fraction=0.5)


def test_split_partitions_all_rows(fake_hub):
    ...
    assert train_ds.n_records > 0 and val_ds.n_records > 0
    assert train_ds.n_records + val_ds.n_records == all_ds.n_records
    assert train_ds.n_tokens + val_ds.n_tokens == all_ds.n_tokens


def test_split_is_deterministic(fake_hub):
    a = TinyfactsDataset(split="val", **SPLIT_KWARGS)
    b = TinyfactsDataset(split="val", **SPLIT_KWARGS)
    assert torch.equal(a.tokens, b.tokens)


def test_split_seed_changes_partition(fake_hub):
    """A different seed sends different rows to val, so the tokens differ."""
    a = TinyfactsDataset(split="val", split_seed=0, **SPLIT_KWARGS)
    b = TinyfactsDataset(split="val", split_seed=42, **SPLIT_KWARGS)
    assert not torch.equal(a.tokens, b.tokens)


def test_rejects_invalid_split(fake_hub):
    with pytest.raises(ValueError, match="split must be"):
        TinyfactsDataset(context_size=CONTEXT_SIZE, split="test")
